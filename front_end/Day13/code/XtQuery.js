;(function( window ){
    ...
    XtQuery.prototype.selector = function( selector ){
    ...
    // 选择器功能 : 
    XtQuery.prototype.eq = function( index ){
        // 1. 找到dom对象! 
    ...
    XtQuery.prototype.end = function(){
        // 返回当前选择器的上一次选择结果; 
        return this.prev;
    }

    XtQuery.prototype.css = function( options ){
    ...
    XtQuery.prototype.html = function( html ){
        for(let i = 0 ; i < this.length ; i ++){
             let dom = this[i];
    ...
    XtQuery.prototype.val = function( val ){
        for(let i = 0 ; i < this.length ; i ++){
             let dom = this[i];
             dom.value = val ; 
         }
    ...
    // 作业 : 
    XtQuery.prototype.attr = function( val ){
        for(let i = 0 ; i < this.length ; i ++){
            let dom = this[i];
            // 在这里可以设置元素的属性! 
